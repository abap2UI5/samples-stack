 ...
 *    Fifteen statements here read a whole RAP or database table with no WHERE -
 *    the travel and booking lists these samples are built around, which is
 ...
 *    `IS ASSIGNED` would then read TRUE for the failure. Two of these shipped
 *    here, both in z2ui5_cl_smps_context's own copy of the message helpers
 *    and both inside a LOOP over an object's public attributes - where a
 *    stale binding maps the PREVIOUS attribute under this one's name.
 *
 ...
 *    sample that picks up a text symbol picks up the trap with it.
 *
